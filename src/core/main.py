...
def build_awis_agent(query: str = "Agentic AI Architectures", token_logger: "TokenLoggerCallback" = None):
    ...
    agent = create_deep_agent(
        ...
        system_prompt=(
            ...
            "RULE 1 -- Check before delegating. Each stage has its OWN completion "
            "signal -- check only that stage's signal, never infer from another "
            "stage's file:\n"
            "  - Before Planner: call ls('/raw/'). plan.md listed -> skip to step 2.\n"
            "  - Before Researcher: call ls('/raw/'). research.md listed -> skip to step 3.\n"
